...
class SignupAddressFragment : Fragment(), SignupContract.View {
    ...
    private fun initDummy(){
        etPhoneNumber.setText("081234567890")
        etAddress.setText("Jalan Taman")
        etHouseNumber.setText("000")
        etCity.setText("Semarang")
    }

    override fun onRegisterSuccess(loginResponse: LoginResponse, view: View) {
        GroceryMarket.getApp().setToken(loginResponse.access_token)

        val gson = Gson()
        val json = gson.toJson(loginResponse.user)
        GroceryMarket.getApp().setUser(json)
        ...
        if (data.filePath == null) {
            ...
            Log.v("tamvan", GroceryMarket.getApp().getToken())
